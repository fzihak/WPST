...
import { Badge, SectionHeader, SeverityBadge, TimeChip } from "@/components/ui";
...
function Hero() {
  ...
  return (
    <section className="relative overflow-hidden border-b border-[var(--color-wp-border-light)]">
      <div className="glow pointer-events-none absolute inset-0" />
      <div className="grid-noise pointer-events-none absolute inset-0 text-[var(--color-wp-text)]" />
      <div className="relative mx-auto max-w-6xl px-4 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-24">
        <div className="max-w-3xl">
          <Badge tone="brand" className="font-mono">
            <Sparkles className="h-3 w-3" /> open source · MIT · {stats.sections} sections · zero backend
          </Badge>
          <h1 className="text-balance mt-5 text-[40px] font-semibold leading-[1.02] tracking-[-0.03em] text-[var(--color-wp-text)] sm:text-[64px]">
            WordPress Support Toolkit
          </h1>
          <p className="text-balance mt-4 text-[20px] font-medium leading-snug tracking-tight text-[var(--color-wp-text-secondary)] sm:text-[26px]">
            The Open Troubleshooting Standard for WordPress.
          </p>
          <p className="mt-5 max-w-2xl text-[15.5px] leading-7 text-[var(--color-wp-text-secondary)]">
            Written for technical support engineers, WordPress developers, agencies and hosting teams. Every article
            starts with <span className="font-semibold text-[var(--color-wp-text)]">how to investigate</span>, not
            with a blind fix — symptoms, evidence, isolation, resolution, verification, prevention.
          </p>
        </div>
        ...
        <div className="relative mt-8 max-w-2xl">
          <form
            ...
            className="card flex items-center gap-2 p-1.5 shadow-lg shadow-black/5"
          >
            <Search className="ml-3 h-4 w-4 shrink-0 text-[var(--color-wp-text-muted)]" />
            <input
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="Search everything: 500, memory, ssl, plugin conflict, payment failed…"
              aria-label="Search the knowledge base"
              className="min-w-0 flex-1 bg-transparent py-2 text-[15px] text-[var(--color-wp-text)] outline-none placeholder:text-[var(--color-wp-text-muted)]"
            />
            <button type="submit" className="btn-primary shrink-0 px-3.5 py-2 sm:px-4">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-1.5">
          ...
          {HERO_CHIPS.map((c) => (
            <Link key={c} to={`/search?q=${encodeURIComponent(c)}`} className="btn-ghost bg-white/70">
              {c}
            </Link>
          ))}
        </div>

        <dl className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-[var(--color-wp-border-light)] bg-[var(--color-wp-border-light)] sm:grid-cols-4">
          {[
            ...
            <div key={s.k} className="bg-white/90 px-5 py-4 backdrop-blur">
              <dt className="font-mono text-[10px] uppercase tracking-widest text-[var(--color-wp-text-muted)]">{s.k}</dt>
              <dd className="mt-1 text-[26px] font-semibold tracking-tight text-[var(--color-wp-text)]">
                {s.v}
                {s.suffix}
              </dd>
            </div>
